 ...
 ** *Streams / sub streams present, but there are no check states* - Confirm that the payload sent to the Receiver API adheres to the xref:/configure/health/send-health-data[health payload specification].
...
* xref:/home/jhk/projects/suse/product-docs/stackstate-product-docs/docs/next/modules/en/setup/cli/cli-sts.adoc[Install the SUSE Observability CLI]
