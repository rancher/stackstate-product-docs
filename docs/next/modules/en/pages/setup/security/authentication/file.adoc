 ...
 ** *roles* - the list of roles that the user is a member of. The link:../rbac/rbac_permissions.adoc#predefined-roles[default SUSE Observability roles] are `stackstate-admin`, `stackstate-power-user` and `stackstate-guest`, for details on how to create other roles, see xref:../rbac/rbac_roles.adoc[RBAC roles].
...
When the user passwords should come from an external secret, follow link:/setup/security/external-secrets.adoc#getting-authentication-data-from-an-external-secret[these steps] but fill in the following data:
