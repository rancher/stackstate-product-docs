...
The main advantage of the configuration backup is that it is very small (usually only several megabytes) and easy and quick to restore with minimal downtime. After the configuration backup is restored new data will be processed as before, recreating topology, health states and alerts. Topology history (including health) are however not preserved. For that purpose there is the xref:kubernetes_backup.adoc[StackGraph backup], however those backups are a lot bigger, and take much longer to create and restore.
...
. Make sure to connect to the context and namespace for SUSE Observability, <<working-with-configuration-backups,see here>>
...
Configuration backups can be stored in object storage, this happens automatically when configuring MinIO and enabling backups for topology, metrics, events and logs. Please follow the instructions for the link:./kubernetes_backup.adoc#enable-backups[Kubernetes backup].
