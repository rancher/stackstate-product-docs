...
. Configure SUSE Observability for using Rancher as an OIDC provider

    Generate your `oidc_values.yaml`. This guide assumes that you save it in the `$VALUES_DIR`
...
[NOTE]
====
For a more detailed explanation of how to configure it follow  xref:/setup/security/authentication/oidc.adoc#_rancher[Configure SUSE Observability for using Rancher as an OIDC provider]
====
