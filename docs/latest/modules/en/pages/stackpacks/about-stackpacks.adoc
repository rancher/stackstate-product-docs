...
If a new StackPack version is available, an alert will be displayed on the SUSE Observability UI StackPack page and you will have the option to view the upgrade details and read the release notes. You can manually upgrade the StackPack from here.

Some StackPacks are configured to upgrade automatically (see `stackstate.stackpacks.upgradeOnStartup` in the {stackstate-product-name} Helm chart), when {stackstate-product-name} starts, for example during a restart or platform upgrade. Automatic StackPack upgrades only apply to patch and minor versions. Major StackPack versions are never upgraded automatically, even when the StackPack is configured for automatic upgrades.

[NOTE]
====
StackPacks can be configured to be upgraded automatically when {stackstate-product-name} starts. This is controlled by the `stackstate.stackpacks.upgradeOnStartup` setting in the {stackstate-product-name} Helm chart.
====
...
Major StackPack versions can contain breaking changes to the StackPack output, such as removed or significantly changed overviews, components, monitors, relations, or presentation. Major versions are never upgraded automatically. Review the release notes and click *UPGRADE NOW* on the StackPack page in the SUSE Observability UI to manually upgrade.
...
. The customized configuration items will now be available in SUSE Observability and can be copied to the upgraded StackPack configuration items.
...
[NOTE]
====
Configuration items shipped by integrations have *immutable ownership* and cannot be locked or unlocked. This applies to items such as monitors, dashboards, views, main menu groups and others.
====
