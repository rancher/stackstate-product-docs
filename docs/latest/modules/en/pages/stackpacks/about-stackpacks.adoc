...
[NOTE]
====
Configuration items shipped by newer (StackPacks 2.0) integrations have *immutable ownership* and cannot be locked or unlocked. This applies to items such as monitors, dashboards, views, component presentations, main menu groups and OpenTelemetry mappings. Rather than changing the item that a StackPack installed, extend it by adding a more specific configuration item of your own.
====
