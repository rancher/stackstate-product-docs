...
[CAUTION]
====
When the metric binding doesn't specify an aggregation {stackstate-product-name} will automatically use the `last_over_time` aggregation to reduce the number of data points for a chart. See also xref:/develop/reference/writing-promql.adoc#_why_aggregate[Why is aggregation necessary?] for an explanation.
====
