 ...
 ** Click *install*.
. Deploy the SUSE Observability Agent, Cluster Agent, Checks Agent and kube-state-metrics on your Cluster using the helm command provided in the SUSE Observability UI after you have installed the StackPack. The default namespace for the agent is `suse-observability-agent`. The UI instructions remove a previous agent release from `suse-observability` before installing into the new namespace to avoid running duplicate agents.
