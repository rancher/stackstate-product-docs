...
[IMPORTANT]
====
A service token must be used instead of API key as they are synonymous. Any environment variable that continues to refer to `receiverApiKey` is for backwards compatibility. For more information on how to set up a bootstrap service token, see xref:/use/security/k8s-service-tokens.adoc#_set_up_a_bootstrap_service_token[Set up a bootstrap service token].
====
...
If you override `imageRegistry` to pull through a private mirror or air-gapped registry, you can still confirm that images have not been tampered with. See xref:setup/security/verify-image-signatures.adoc[Verify container image signatures].
