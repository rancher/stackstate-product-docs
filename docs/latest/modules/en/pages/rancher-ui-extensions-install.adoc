...
[WARNING]
====
The service token must be deleted when the extension no longer uses it to prevent unauthorized access to data from {stackstate-product-name}.
====

----
sts service-token create --name suse-observability-extension --roles stackstate-guest
----
