...
. Generate a self-signed certificate <<generate-a-certificate-locally,Generate a certificate locally>>.
. Use the k8s https://cert-manager.io/[cert-manager] (if it already on the cluster) <<generate-a-certificate-using-the-cert-manager,Generate a certificate using the cert-manager>>.
