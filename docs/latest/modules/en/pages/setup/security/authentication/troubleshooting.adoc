...
For SUSE Observability installations whose Rancher preset cannot configure scopes, use the following workaround.
Do not request `groups` from Rancher 2.12 or 2.13.

. Update the existing OIDCClient in the Rancher local cluster to allow the required scopes.
  Replace `oidc-observability` with your client name.
  If the client allows additional scopes, preserve them in the list below.
  This patch replaces the scope list.
+
[,bash]
----
...
Requesting `groups` before allowing it on the client can prevent login with `invalid_scope`.

. Remove `stackstate.authentication.rancher` from your SUSE Observability values and replace it with the generic OIDC configuration below.
...
. Apply the values with your usual Helm upgrade procedure.
