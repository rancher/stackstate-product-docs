...
* **oidcLogout** - Optional (default `false`): when set to `true`, logging out of SUSE Observability also logs the user out at Rancher (OIDC RP-initiated logout / single sign-out). When `false`, logging out only ends the SUSE Observability session.
* **skipLoginPage** - Optional (default `false`): when set to `true`, the SUSE Observability login page is skipped and users are redirected straight to Rancher. Only enable this when an OIDC provider is configured; without one there would be no way to reach the login form.
...
To configure SUSE Observability to use any other OIDC provider, you need to add OIDC details and user role mapping to the file `authentication.yaml`. For example:
 ...
 ** *oidcLogout* - Optional (default `false`): when set to `true`, logging out of SUSE Observability also logs the user out at the OIDC provider (OIDC RP-initiated logout / single sign-out). When `false`, logging out only ends the SUSE Observability session.
 ** *skipLoginPage* - Optional (default `false`): when set to `true`, the SUSE Observability login page is skipped and users are redirected straight to the OIDC provider. Only enable this when an OIDC provider is configured; without one there would be no way to reach the login form.
