 ...
 ** *oidcLogout* - Optional (default `false`): when set to `true`, logging out of {stackstate-product-name} also logs the user out of KeyCloak. When `false`, logging out only ends the {stackstate-product-name} session.
 ** *skipLoginPage* - Optional (default `false`): when set to `true`, the {stackstate-product-name} login page is skipped and users are redirected straight to KeyCloak.
...
Setting `oidcLogout: false` and `skipLoginPage: true` will cause the logout process to immediately redirect to KeyCloak and re-authenticate, which will make it appear that the user was not logged out.
====
...
For configuring SSO identity providers in Keycloak (for example, Microsoft Entra ID, Google, or other OIDC providers), see the xref:/suse-cloud-observability/user-management.adoc[SUSE Cloud Observability User Management] SSO setup guides. The Keycloak-side configuration steps are the same for self-hosted and SaaS deployments. Self-hosted users additionally need to map the resulting Keycloak roles in `authentication.yaml` using the `jwtClaims.groupsField` and `roles` settings described above.
