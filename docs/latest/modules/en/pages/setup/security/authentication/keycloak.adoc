...
== SSO identity provider setup guides

When using Keycloak as the authentication provider for {stackstate-product-name}, you can configure Keycloak to delegate authentication to an external SSO identity provider. The following guides describe how to set up specific providers in Keycloak:
