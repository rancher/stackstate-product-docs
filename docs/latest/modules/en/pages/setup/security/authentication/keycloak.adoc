 ...
 ** *oidcLogout* - Optional (default `false`): when set to `true`, logging out of SUSE Observability also logs the user out at KeyCloak (OIDC RP-initiated logout / single sign-out). When `false`, logging out only ends the SUSE Observability session.
 ** *skipLoginPage* - Optional (default `false`): when set to `true`, the SUSE Observability login page is skipped and users are redirected straight to KeyCloak. Only enable this when KeyCloak (or another OIDC provider) is configured; without an OIDC provider there would be no way to reach the login form.
