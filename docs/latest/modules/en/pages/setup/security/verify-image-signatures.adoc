...
Image signing was introduced for platform images in release v2.10.4 and for agent images in release vA.B.C. Images published before this was rolled out do not carry a signature — verifying one of those tags fails with `no signatures found`, which does not indicate a problem with the image itself. If you rely on signature verification, pull a current tag.
...
`no signatures found`::
This is either an older tag published before we started signing images (see <<_overview,Overview>>) or a mistyped registry, repository, or tag.
