...
You can assign the *Role Templates* to users or groups in the *Project* that is running {stackstate-product-name}.  These roles grant permission to use and configure the system,
including setting up views, monitors, notifications and so on.  With any of these roles, it also becomes possible to use the CLI.  It is not necessary to do this for all users.  

=== Instance roles with access to observability data

A couple of "global" roles allow access to all observability data - in any of the observed clusters.  These roles are intended to be used for setting up the system and for troubleshooting system-level problems.
