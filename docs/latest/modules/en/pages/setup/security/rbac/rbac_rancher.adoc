...
If Rancher Manager is deployed using the https://ranchermanager.docs.rancher.com/reference-guides/best-practices/rancher-server/rancher-deployment-strategy#regional-strategy[Regional Strategy], it is still possible to use RBAC in a shared SUSE Observability instance.  For this to work

* all Rancher instances must share an upstream OIDC provider (e.g. Keycloak), and 
* one of the Rancher instances must be the OIDC provider for SUSE Observability

Each Rancher instance performs the same mapping from upstream (Keycloak) users and groups to their Kubernetes counterparts.

[WARNING]
====
Shipping observability data from one region (the one running the downstream Kubernetes cluster) to another (the cluster running SUSE Observability) may incur a significant cost.
====
