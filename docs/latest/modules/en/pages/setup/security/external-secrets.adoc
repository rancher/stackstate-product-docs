...
{stackstate-product-name} can take secrets like license key, service token and authentication keys through the helm install, but can also take those from already provisioned secrets.
...
[IMPORTANT]
====
A service token must be used instead of API key as they are synonymous. Any environment variable that continues to refer to `API_KEY` is for backwards compatibility. For more information on how to set up a bootstrap service token, see xref:/use/security/k8s-service-tokens.adoc#_set_up_a_bootstrap_service_token[Set up a bootstrap service token].
====

Create a secret in the namespace {stackstate-product-name} is installed in, of the following form, filling in the blanks:
