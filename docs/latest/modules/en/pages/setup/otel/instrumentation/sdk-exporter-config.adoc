...
To send data to {stackstate-product-name} the SDKs that are used to instrument your application use a built-in exporter. A production ready setup uses <<with-a-collector-production-setup,a collector>> close to your instrumeneted applications to send the data to SUSE Observability, but it is also possible to have the instrumented application <<without-a-collector,directly send>> the telemetry data to SUSE Observability.
