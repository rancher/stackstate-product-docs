...
The example uses the probabilistic sampler configured to sample 25% percent of the traffic. You must tune the percentage for your situation or switch to the <<tail-sampling,tail sampler>> instead. The pipeline setup is the same for the tail sampler, just replace the reference to the `probabilistic_sampler` with `tail_sampling`.
