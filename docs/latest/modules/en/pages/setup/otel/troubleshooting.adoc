...
There are a lot of configuration options but more importantly, every (Kubernetes) environment is slightly different. To find out where the problem is, the quickest approach is to pick a pod from which telemetry data is expected using the following steps:

. Check the beginning of the logs for the pod. SDKs log warnings or errors when instrumentation fails at startup.
. Check the logs for any errors related to sending data to the collector.
. Check the logs of the collector pod(s) for configuration or initialization errors. These are logged right after the startup of the pod.
. Check the collector logs also for errors related to sending data to {stackstate-product-name}.

The error(s) in the logs usually give a good indication of the problem. The most common causes for Open Telemetry data not being available for a few or all your instrumented applications are listed. If the problem is not listed here, you can also look at the https://opentelemetry.io/docs/languages/[language-specific SDK documentation] or the https://opentelemetry.io/docs/collector/troubleshooting/[collector documentation] from Open Telemetry.
...
When different names are used for the same cluster, {stackstate-product-name} cannot match the data from Open Telemetry with the data from the {stackstate-product-name} agent and the traces perspective remains empty.
...
=== Certain proxies and firewall rules do not work well with gRPC

If the collector needs to send data to {stackstate-product-name} through a proxy or a firewall it can be that they either block the traffic or remove certain parts of the gRPC messages or fully remove the long-lived gRPC connection. The easiest fix is to switch from gRPC to use HTTP instead, by replacing the `otlp/suse-observability` exporter configuration **and the references to it in the pipelines section** with the `otlphttp/suse-observability` exporter.
