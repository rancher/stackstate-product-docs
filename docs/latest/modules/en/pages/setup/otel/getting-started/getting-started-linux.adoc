...
[NOTE]
====
This Linux guide uses a self-managed Open Telemetry collector. The telemetry gateway in the {stackstate-product-name} Agent is the recommended path for Kubernetes workloads (see xref:/setup/otel/getting-started/getting-started-k8s.adoc[Kubernetes getting started]).
====
...
* Forward the data to SUSE Observability, including authentication using the service token
...
sudo rpm -iv1 otelcol-contrib_0.151.0_linux_amd64.rpm
----
...
sudo rpm -iv1 otelcol-contrib_0.151.0_linux_arm64.rpm
----
