...
[NOTE]
====
This Lambda guide uses a self-managed Open Telemetry collector proxy. The telemetry gateway in the {stackstate-product-name} Agent is the recommended path for Kubernetes workloads running in-cluster (see xref:/setup/otel/getting-started/getting-started-k8s.adoc[Kubernetes getting started]).
====
