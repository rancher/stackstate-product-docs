...
This section provides information on monitoring an application.
...
Install the OTel (Open Telemetry) collector in cluster A. Configure it to:
...
* **SUSE Observability UI** - Open the main menu by clicking the top-left of the screen and navigating to `StackPacks` -> `Open Telemetry`.  If you have not done so before, click the `INSTALL` button. Click the `CREATE NEW SERVICE TOKEN` button and copy the value onto your clipboard.
* **SUSE Observability CLI** - For more information, see xref:/use/security/k8s-service-tokens.adoc#_manage_service_tokens[Manage service tokens].
...
Install the collector with a Helm chart provided by the Open Telemetry project. Make sure you have the Open Telemetry helm charts repository configured:
