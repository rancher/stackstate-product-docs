...
* The SUSE Observability Agent telemetry gateway running in cluster A, receiving OTLP data from the observed application(s) and sending it to {stackstate-product-name}.
* {stackstate-product-name} running in cluster B, or SUSE Cloud Observability.

image::otel/open-telemetry-collector-kubernetes.png[Container instrumentation with the SUSE Observability Agent telemetry gateway]
...
Enable the OTel pipeline and the SUSE Observability Agent telemetry gateway in your {stackstate-product-name} Agent values:
...
For other languages follow the documentation on https://opentelemetry.io/docs/languages/[opentelemetry.io] and configure the SDK exporter to ship data to the SUSE Observability Agent telemetry gateway by following xref:/setup/otel/instrumentation/sdk-exporter-config.adoc[these instructions].
