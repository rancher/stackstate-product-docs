...
Let's create an `otel-operator.yaml` file to configure the operator:
...
manager:
  ...
  collectorImage:
    ...
    # Example tag, update with the latest version
...
=== Create a cluster role and cluster role binding for the collector

The OTel collector configuration we use in this guide includes a k8sattributes processor that enriches the telemetry data with Kubernetes metadata to have the best experience in SUSE Observability. This processor needs permissions to query the Kubernetes API for this metadata, so we need to create a service account, cluster role and cluster role binding for the collector.

Create a file `otel-collector-rbac.yaml` with the following content, we're again using `open-telemetry` as the namespace:
...
Now apply this file to create the cluster role and cluster role binding:
