...
We install the operator with the Helm chart provided by the link:https://apps.rancher.io/applications/opentelemetry-operator[SUSE Application Collection]. Please check their link:https://docs.apps.rancher.io/get-started/authentication[authentication guide] to configure Application Collection OCI credentials in your Kubernetes cluster.

By default, OpenTelemetry Operator for Kubernetes depends on cert-manager to issue and renew the self-signed certificate for the webhook component. You can install it from link:https://docs.apps.rancher.io/reference-guides/opentelemetry-operator#install-cert-manager[Application Collection] if you do not have it installed or disable the cert-manager dependency entirely as we do in the command below.

Install the operator using Application Collection image pull secret named `application-collection` (link:https://docs.apps.rancher.io/get-started/authentication[Application Collection authentication guide]):
...
The operator manages one or more collector deployments via a Kubernetes custom resource of kind `OpenTelemetryCollector`. This guide shows the collector-based path for operator-managed collection. If you already use the {stackstate-product-name} Agent telemetry gateway, you can also configure the `Instrumentation` exporter endpoint to point at that gateway instead of an operator-managed collector (see xref:/setup/otel/getting-started/getting-started-k8s.adoc[Kubernetes getting started]).
...
```
kubectl create serviceaccount --namespace open-telemetry otel-collector
kubectl patch serviceaccount --namespace open-telemetry otel-collector --patch '{"imagePullSecrets": [{"name": "application-collection"}]}'
```
...
* link:https://opentelemetry.io/docs/platforms/kubernetes/operator/[Open Telemetry Operator]
* link:https://opentelemetry.io/docs/platforms/kubernetes/operator/automatic/[Open Telemetry Operator auto-instrumentation]
* link:https://docs.apps.rancher.io/reference-guides/opentelemetry-operator[Open Telemetry Operator reference guide from SUSE Application Collection]
