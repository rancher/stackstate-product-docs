...
The `output.identifier` field defines the sole identity of a component. All components produced with the same identifier are considered the same logical entity and are merged by the platform. This makes identifier construction a critical design choice. 
...
input:
  signal:
    - "TRACES"
  resource:
    condition: "${'service.name' in resource.attributes}" # action omitted since the default is `CONTINUE`
    scope:
...
* When there are multiple input signals declared, data from within an expression can only be accessed from the common-ancestor-level, which in all cases are scope-level.
* Data/field access from another level is not allowed. For example, scope-level fields cannot be accessed from resource-level and vice versa.
...
input:
  ...
  resource:
    condition: "${'service.name' in resource.attributes}" # action omitted since the default is `CONTINUE`
    scope: # action and condition have been omitted since the defaults are `CONTINUE` and `true` respectively
      span:
        action: "CREATE"
...
input:
  signal:
    - "METRICS"
  resource: # action and condition have been omitted since the defaults are `CONTINUE` and `true` respectively
    scope:
      condition: "scope.name == 'traces_service_graph'"
      metric: # action omittted since the default is `CONTINUE`
