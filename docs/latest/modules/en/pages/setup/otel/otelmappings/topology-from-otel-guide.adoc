...
This document provides a getting started guide for OTel component and relation mappings. It walks you through creating a service-level topology from OpenTelemetry trace data, (i.e., local span context) using component and relation mappings, packaged and tested as part of a StackPack.

At the end of this guide, you will have:

* Defined topology components derived from OpenTelemetry traces
* Created relationships that represent synchronous service calls
* Validated the resulting topology using the StackPack test workflow
...
We want to visualize a simple service topology based on how services interact at runtime. Specifically, we want to model the following flow:
...
All of this topology will be derived automatically from OpenTelemetry trace data.

== The trace data that we'll work with

Throughout this guide, we’ll use a simplified OTLP-style JSON trace payload, aligned with the official OpenTelemetry Protocol (OTLP) https://github.com/open-telemetry/opentelemetry-proto/blob/main/opentelemetry/proto/trace/v1/trace.proto[trace] schema.
...
== From traces to topology: the mental model
...
* Familiarize yourself with an in-depth component and relation mapping xref:/setup/otel/otelmappings/schemas.adoc[reference]
