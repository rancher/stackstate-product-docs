...
* Identified by `process.pid` and executable metadata.
...
2. In the left-hand navigation-bar, click on `Open Telemetry > Services Instances`
...
See the xref:/setup/otel/otelmappings/troubleshooting.adoc[Troubleshooting] guide for tips to debug if the topology is not materializing as expected.
...
1. Open the {stackstate-product-name} UI at the configured `baseUrl` Helm value
2. In the left-hand navigation-bar, click on `Open Telemetry > Services Instances`
3. In the sub navigation-bar, click on `Save view as...` and give it a name (e.g., `Custom process components`)
4. In the left-hand navigation-bar, click on `Views`, and open the view created in step 3
5. In the sub navigation-bar, on the far right, click on `Filters`
...
* Created execution topology without span correlation.

=== Known limitations

* Process identifiers are scoped to their runtime environment.
* Short-lived processes may expire quickly.
* Cross-host process relationships are not modeled.
