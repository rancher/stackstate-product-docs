...
[IMPORTANT]
====
A service token must be used instead of API key as they are synonymous. Any environment variable that continues to refer to `OTLP_API_KEY` is for backwards compatibility.
====
...
* *Secret Management:* Securely store sensitive information like service tokens.
...
----
VERBOSITY: "normal"
OTLP_API_KEY: "<your service token for sending data to SUSE Observability here>"
...
For more information on how to set up a batching collector that acts as a security proxy for {stackstate-product-name}, see xref:/setup/otel/getting-started/getting-started-lambda.adoc[Getting started with Lambda]. 
