...
presentation:
  ...
  tracesPerspective:                              # Optional - makes the Traces perspective available
    ...
    spanFilter:                                   # Selects the spans shown in the perspective
      serviceName: [cel:string]                   # Optional - filter spans by service name
      spanName: [cel:string]                      # Optional - filter spans by name
      attributes:                                 # Optional - filter spans by one or more attributes
        <attribute-key>: [cel:string]
      spanKind: [Client | Consumer | Internal | Producer | Server | Unspecified]
      spanParentType: [External | Internal | Root]
      ...
      scopeName: [cel:string]                     # Optional - filter by instrumentation scope name
      scopeVersion: [cel:string]                  # Optional - filter by instrumentation scope version
...
The `tracesPerspective` section makes the Traces perspective available on the highlight page of matching components.  When the section is absent, the Traces perspective is not shown for that component.

`spanFilter` selects which spans appear in the perspective.  String-valued fields are CEL expressions evaluated against the component, using the same context as metric queries — for example `tags.singleValue("service.name")` or `name`.  Two things to keep in mind when writing them:

* `tags` maps each key to a *list* of values, so use `singleValue` rather than indexing directly when a string is required.
* Literal values must be written as quoted CEL strings, for example `'orderService'`.  A bare word is parsed as an identifier and will not resolve.
