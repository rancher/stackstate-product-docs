...
[NOTE]
====
The component identifier also links spans back to this component in the Traces perspective. When a span is selected, its resource attributes, span attributes, span name, kind, status, and scope name/version are all used to resolve matching mappings — so identifiers that reference span-level fields (for example, `span.attributes['db.name']`) resolve correctly for database components. Identifiers that depend on metric- or log-level data are not resolved for this. See xref:/setup/custom-integrations/otelmappings/troubleshooting.adoc[troubleshooting] if a span shows no linked component.
====
...
* Otherwise the side with the higher `specificity` wins.

`specificity` is declared under the required `rank` block in each `OtelComponentMapping` (as `rank.specificity`). Use a higher value to specialise a more specific mapping over a more general one for the same component identifier.
...
Higher specificity wins when multiple mappings contribute to the same component identifier.
...
A tag key can have zero, one, or many values, for example:
