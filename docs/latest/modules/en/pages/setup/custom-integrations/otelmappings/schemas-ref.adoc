...
* `tags['key']` — single-value access. Returns the value of the matching tag. If a key has multiple values (for example, after merging two contributing sources that both produced a `source-type` label), the values are joined with `,` (sorted) so existing templates that expect a string keep working.
* `tagsAll['key']` — multi-value access. Returns the full list of values for the key, including duplicates if any. Useful for:
