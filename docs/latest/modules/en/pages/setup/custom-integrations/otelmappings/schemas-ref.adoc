...
[NOTE]
====
The component identifier also links spans back to this component in the Traces perspective: when it resolves from a span's resource attributes, that span's details link to the matching component. Identifiers that depend on span-, metric-, or log-level data are not resolved for this. See xref:/setup/custom-integrations/otelmappings/troubleshooting.adoc[troubleshooting] if a span shows no linked component.
====
