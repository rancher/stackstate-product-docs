...
specificity: float                  # Used during component merge to pick the winning typeName / name
----
...
`specificity` is declared at the top level of each `OtelComponentMapping`. Use a higher value to specialise a more specific mapping over a more general one for the same component identifier. Default is `0.0`; mappings without an explicit value behave as `0.0`.
...
These labels are injected at synthesis time, so they appear on every component (single-source as well as merged). Query them with STQL (for example, `label = 'source-type:pod'`) or, in CEL contexts, with the multi-value accessor `tagsAll['source-type']` described in <<Multi-value tag access in CEL>>.

[NOTE]
====
Duplicate values for the same key (for example, two sources both contributing `source-type:pod`) are currently retained as repeated labels. A future improvement will dedupe identical values; today only the multi-value access path is required to handle the duplicates explicitly.
====
...
Component tags are exposed to CEL expressions through two accessors:

* `tags['key']` — single-value / display-only access. Returns the value of the matching tag. If a key has multiple values (for example, after merging two contributing sources that both produced a `source-type` label), the values are sorted and joined with `,`. This is intended for display and PromQL template interpolation where a plain string is expected. Do not attempt to split or parse the result: a tag value may itself contain `,`, making the output ambiguous.
* `tagsAll['key']` — multi-value access. Returns the full list of values for the key, including duplicates if any. Use this whenever you need to reason about individual values. Useful for:
** set-membership matching, e.g., ``'pod' in tagsAll['source-type']``
** explicit join control, e.g., ``tagsAll['source-type'].join('|')``

[,yaml]
----
# Single-value access — returns 'pod,virt-launcher' if both sources contributed
${tags['source-type']}

# Multi-value access — returns ['pod', 'virt-launcher']
${tagsAll['source-type']}

# Membership test — true if any contributing source produced source-type:pod
${'pod' in tagsAll['source-type']}
----

Prefer `tagsAll` whenever you intend to react to *any* of several merged values; reserve `tags` for places where a single string is required (for example, templated PromQL or STQL where the resulting value is interpolated into another query).
