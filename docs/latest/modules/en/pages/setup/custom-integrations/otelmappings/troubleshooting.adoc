...
== A span shows no linked component in the Traces perspective

In the xref:/use/views/k8s-traces-perspective.adoc[Traces perspective], span details link back to the topology component(s) a span belongs to. For Open Telemetry mappings, this link is xref:/setup/custom-integrations/otelmappings/concepts.adoc#_span_to_component_links_in_the_traces_perspective[derived from the component mappings] by resolving each mapping's component identifier against the span's *resource* attributes. If a span shows no linked component, check the following:

* *The component identifier depends on more than resource attributes.* Only resource-level attributes are available when linking a span. If the identifier - or a variable it references - depends on span, metric, log, datapoint or scope data, it cannot be resolved from the span's resource attributes and the mapping is skipped for this purpose. Rework the identifier so it depends only on `resource` attributes if you want it to contribute a link.
* *The required resource attributes are missing on the span.* The identifier can only be built when the resource attributes it references are present. Confirm the span's resource carries those attributes.
* *No matching component exists.* The resolved identifier must match an existing topology component. Verify the component was created, for example with the xref:/setup/custom-integrations/otelmappings/troubleshooting.adoc#_show_an_open_telemetry_component_or_relation_mappings_status[mapping status command].
* *The mapping does not parse.* A component identifier that fails to parse is skipped and logged by the platform at warning level. Fix the reported mapping errors.
