...
In the xref:/use/views/k8s-traces-perspective.adoc[Traces perspective], span details link back to the topology component(s) a span belongs to. For Open Telemetry mappings, this link is xref:/setup/custom-integrations/otelmappings/schemas-ref.adoc#_component_identity[derived from the component mappings] by resolving each mapping's component identifier against the span's resource and span attributes, plus span name, kind, status, and scope. If a span shows no linked component, check the following:

* *The component identifier depends on metric, log, or datapoint data.* Resource attributes, span attributes, span name, span kind, status code, status message, scope name, and scope version are all available when linking a span. If the identifier — or a variable it references — depends on metric, log, or datapoint data, it cannot be resolved and the mapping is skipped for this purpose.
* *The required attributes are missing on the span.* The identifier can only be built when the attributes it references are present. Confirm the span carries those resource or span attributes.
