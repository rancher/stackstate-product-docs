...
In the xref:/use/views/k8s-traces-perspective.adoc[Traces perspective], span details link back to the topology component(s) a span belongs to. For Open Telemetry mappings, this link is xref:/setup/custom-integrations/otelmappings/schemas-ref.adoc#_component_identity[derived from the component mappings] by resolving each mapping's component identifier against the span's *resource* attributes. If a span shows no linked component, check the following:
