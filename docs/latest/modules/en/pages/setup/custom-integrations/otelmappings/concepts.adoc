...
When you open a span in the xref:/use/views/k8s-traces-perspective.adoc[Traces perspective], {stackstate-product-name} links it back to the topology component it belongs to. For StackPacks that ship Open Telemetry mappings, this link comes from the component mappings themselves: the same component identifier a mapping uses to build topology is resolved against the span's resource attributes to find the matching component, so you do not need to configure anything extra.

Only a span's resource attributes are available for this, so a mapping contributes a link only when its component identifier resolves from those attributes; mappings that rely on span-, metric-, or log-level data do not. See xref:/setup/custom-integrations/otelmappings/troubleshooting.adoc[troubleshooting] if a span shows no linked component.
