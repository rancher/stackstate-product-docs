...
Generic fields:: (e.g., name, health) should use low order numbers (e.g., 10-20) to appear at the top by default.
Specialized fields:: should use higher order numbers to appear after generic ones.
...
The intention is for filters to persist when navigating between overview and highlight pages.  So if at all possible, try to reuse the same filter ids (with the same semantics) across different component  presentations.  The filters defined for all OTEL components are
