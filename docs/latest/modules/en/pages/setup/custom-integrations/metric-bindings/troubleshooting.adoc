...
* <<the-metric-chart-doesnt-show-on-the-highlights-page-of-a-component,The metric chart does not show on the Highlights page of a component>>
* <<the-metric-chart-doesnt-show-on-the-metrics-perspective-of-a-component,The metric chart does not show on the metrics perspective of a component>>
* <<the-metric-chart-on-a-component-remains-empty-no-data,The metric chart on a component remains empty ("no data")>>

[#the-metric-chart-doesnt-show-on-the-highlights-page-of-a-component]
== The metric chart does not show on the Highlights page of a component
...
[#the-metric-chart-doesnt-show-on-the-metrics-perspective-of-a-component]
== The metric chart does not show on the metrics perspective of a component

The `scope` query on a metric binding is used to determine whether a component shows the metric binding. If a component does not show a metric binding check that the topology query in the scope matches the component.
...
Make sure the query does not contain any of the parameters anymore (i.e. all values like `${tags.cluster-name}` or `+${name}+`) have been replaced with the values for the component. If some parameters were left behind in the query the labels were not available on this component. So cross-check the names used (in this example `cluster-name`) against the labels available on the component. Also make sure there are no typos in the names.
