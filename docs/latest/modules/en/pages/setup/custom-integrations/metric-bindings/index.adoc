...
Create a new YAML file called `metricbindings.yaml` and add this YAML template to it to create your own metric binding. If you used the `sts stackpack` command, this file will already be present.  Open it in your favorite code editor to change it throughout this guide. At the end, the {stackstate-product-name} CLI will be used to create and update the metric bindings in {stackstate-product-name}.
...
Creating it in {stackstate-product-name} and viewing the "Replica count" chart on a deployment component gives an unexpected result. The chart shows the replica counts for all deployments. Logically one would expect only 1 time series: the replica count for this specific deployment.
...
Use the {stackstate-product-name} CLI to create the metric binding in {stackstate-product-name}. Make sure the `metric-bindings.yaml` is saved and looks like this:
