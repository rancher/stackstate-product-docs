...
. <<create-an-outline-of-the-metric-binding,Create an outline of the metric binding>>
. <<select-the-components-to-bind-to,Select the components to bind to>>
. <<write-the-promql-query,Write the PromQL query>>
...
Open the `metricbindings.yaml` YAML file in your favorite code editor to change it throughout this guide. You can use `sts stackpack test-deploy` to test the StackPack, as described in xref:/setup/custom-integrations/create.adoc#_test_deploy_the_stackpack[Test deploy the StackPack].
