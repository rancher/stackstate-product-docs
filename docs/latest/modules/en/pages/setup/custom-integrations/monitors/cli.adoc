...
[NOTE]
====
The recommended way of working is to store monitors (and any other custom resources created in {stackstate-product-name}) as YAML files in xref:/setup/custom-integrations/stackpacks.adoc[a StackPack]. From there changes can be manually applied or it can be fully automated by using the {stackstate-product-name} CLI in a CI/CD system like GitHub Actions or GitLab pipelines.
====
...
You can xref:/setup/custom-integrations/monitors/troubleshooting.adoc#verifying-the-results-of-a-monitor[verify the results of the monitor] on the monitor overview page.
