...
Patch and minor versions can be upgraded automatically when the StackPack is configured for automatic upgrades. Major versions are supported as StackPack upgrades, but they are never upgraded automatically. Users must manually start the upgrade.
