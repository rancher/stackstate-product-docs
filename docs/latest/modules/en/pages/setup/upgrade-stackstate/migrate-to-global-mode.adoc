...
[NOTE]
====
* The `adminPassword` must be a bcrypt hash in the new method. See <<admin-password,Admin password>> for how to generate one.
...
If you prefer using a values file, create `values.yaml` with the configuration shown in <<create-new-values-file,Create new values file>>, ensure you have the latest chart with `helm repo update`, then run:
