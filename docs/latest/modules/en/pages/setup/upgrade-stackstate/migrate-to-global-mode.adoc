...
[NOTE]
====
* The `adminPassword` must be a bcrypt hash in the new method. See <<_generate_a_bcrypt_password_hash>> for how to generate one.
* For air-gapped environments or custom registries, see xref:/k8s-suse-rancher-prime-air-gapped.adoc[Installing SUSE Observability in Air-Gapped Mode] for additional configuration including `imageRegistry` and `pullSecret` settings.
====

=== Using a values file instead

If you prefer using a values file, create `values.yaml` with the configuration shown in <<_step_3_create_new_values_file>>, then run:
...
[NOTE]
====
For air-gapped environments or custom registries requiring `imageRegistry` and `pullSecret` configuration, see xref:/k8s-suse-rancher-prime-air-gapped.adoc[Installing SUSE Observability in Air-Gapped Mode].
====
