...
This subtree was deprecated in earlier {stackstate-product-name} releases. Previous chart versions kept the legacy keys working as a fallback and emitted a deprecation banner during `helm upgrade` that listed every `experimental.*` key alongside its `features.*` replacement. The fallback and the banner are removed in 2.10.0; setting any `stackstate.experimental.*` key now fails the upgrade.
