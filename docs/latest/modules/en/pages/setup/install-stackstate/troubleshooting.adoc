...
=== RBAC agent fails to start with SCC violations

*Symptoms:* The `kubernetes-rbac-agent` pod fails to start on OpenShift with SecurityContextConstraints (SCC) violations. Pod events show errors about `runAsUser`, `fsGroup`, or `seccompProfile` being rejected.

*Affected versions:* Chart versions 1.5.46 and later.

*Cause:* The RBAC agent chart uses hardcoded UID/GID/fsGroup values that conflict with OpenShift's `restricted-v2` SCC requirements.

*Solution:* Enable OpenShift compatibility mode in your `openshift-values.yaml` file:
...
This allows OpenShift's `restricted-v2` SCC to assign UID/fsGroup automatically from the namespace range.
