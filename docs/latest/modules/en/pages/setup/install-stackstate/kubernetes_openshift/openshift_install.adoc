...
[IMPORTANT]
====
A service token must be used instead of API key as they are synonymous. Any environment variable that continues to refer to `receiverApiKey` is for backwards compatibility.
====
...
[IMPORTANT]
=====
Although you can still specify a legacy API key for the `receiverApiKey` variable, you are recommended to use bootstrap service tokens.
=====
...
global:
  ...
  suseObservability:
    ...
    # Optional: Receiver service token (auto-generated if not provided)
...
| Receiver service token
| `receiverApiKey`
| The service token used by {stackstate-product-name} to receive data from agents. This is a secret key that should be kept private. If you omit this, a random key will be generated for you. Although you can still specify a legacy API key for the `receiverApiKey` variable, you are recommended to use *bootstrap service tokens*. Any environment variable that continues to refer to `receiverApiKey` is for backwards compatibility.
...
[NOTE]
=====
Store the generated `basicConfig.yaml` and `sizing_values.yaml` files safely. You can reuse these files for upgrades, which saves time and ensures that {stackstate-product-name} continues to use the same service token. This is desirable as it means Agents and other data providers for {stackstate-product-name} won't need updating.
...
scc:
  enabled: true
# Enable OpenShift compatibility for RBAC agent (chart >= 1.5.46)
...
[NOTE]
====
The `kubernetes-rbac-agent.openshift.enabled` setting is required for chart versions 1.5.46 and later when using the Kubernetes Authorization feature. This allows OpenShift's `restricted-v2` SecurityContextConstraints to assign UID/fsGroup automatically, preventing SCC violations.
====
...
. Before you deploy:
 ** xref:/setup/install-stackstate/kubernetes_openshift/openshift_install.adoc#_create_project[Create the project where {stackstate-product-name} will be installed]
 ** <<create-values-yaml-configuration-file,Create `values.yaml` configuration file>>
 ** <<create-openshift-values-yaml,Create `openshift_values.yaml`>>
...
. Before you deploy:
 ** xref:/setup/install-stackstate/kubernetes_openshift/openshift_install.adoc#_create_project[Create the project where {stackstate-product-name} will be installed]
 ** <<create-values-yaml-configuration-file,Generate `baseConfig_values.yaml` and `sizing_values.yaml`>>
 ** <<create-openshift-values-yaml,Create `openshift_values.yaml`>>
. Deploy the latest {stackstate-product-name} version to the `suse-observability` namespace with the following command:
