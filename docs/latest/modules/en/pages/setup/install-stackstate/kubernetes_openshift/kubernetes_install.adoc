...
[NOTE]
====
For more information about sizing profiles, see xref:setup/install-stackstate/requirements.adoc[System requirements and sizing].
====
...
[IMPORTANT]
=====
Although you can still specify a legacy API key for the `receiverApiKey` variable, you are recommended to use bootstrap service tokens. For more information on how to set up a bootstrap service token, see xref:/use/security/k8s-service-tokens.html#_set_up_a_bootstrap_service_token[Set up a bootstrap service token].
=====
...
| Receiver API key
| `receiverApiKey`
| The API key used by {stackstate-product-name} to receive data from agents. This is a secret key that should be kept private. If you omit this, a random key will be generated for you. Although you can still specify a legacy API key for the `receiverApiKey` variable, you are recommended to use *bootstrap service tokens*. Any environment variable that continues to refer to `receiverApiKey` is for backwards compatibility.
