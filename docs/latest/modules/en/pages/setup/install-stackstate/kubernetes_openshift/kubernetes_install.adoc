...
| `receiverApiKey`
| The service token used by {stackstate-product-name} to receive data from agents. This is a secret key that should be kept private. If you omit this, a random key will be generated for you.
...
[NOTE]
=====
Store the generated `basicConfig.yaml` and `sizing_values.yaml` files safely. You can reuse these files for upgrades, saving time and ensuring that {stackstate-product-name} continues to use the same service token. This is desirable as it means Agents and other data providers for {stackstate-product-name} won't need updating.
