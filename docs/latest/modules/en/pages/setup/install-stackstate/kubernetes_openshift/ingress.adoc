...
Traefik can be used as an alternative to Nginx as an ingress controller. 

=== Known issues

* Make sure to use Traefik <3.6.3 or >=3.6.7 (with Helm chart version >= 39.0.0), as versions 3.6.3 to 3.6.6 have an xref:https://github.com/traefik/traefik/pull/12360#issuecomment-3729140469[issue] that causes errors on some API endpoints. In particular the component highlight pages fail to load due to the API calls being blocked by Traefik with a 400 Bad Request error.
