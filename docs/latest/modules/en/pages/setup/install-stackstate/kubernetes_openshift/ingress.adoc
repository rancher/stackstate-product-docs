...
{stackstate-product-name} can be exposed outside of the cluster using either a traditional https://kubernetes.io/docs/concepts/services-networking/ingress/[Kubernetes Ingress] resource or the https://gateway-api.sigs.k8s.io/[Kubernetes Gateway API]. The examples on this page show how to configure both methods using the {stackstate-product-name} Helm chart. This page also documents which service/port combination to expose when using a different method of configuring ingress traffic.

The https://gateway-api.sigs.k8s.io/[Gateway API] is the successor to Kubernetes Ingress and provides more expressive, extensible, and role-oriented routing. It is recommended for new deployments on clusters that have a Gateway API-compatible controller installed (such as Traefik, Envoy Gateway, or Istio).

[NOTE]
====
`ingress.enabled` and `gateway.enabled` are mutually exclusive -- the Helm chart will fail validation if both are set to `true`.
====
...
== Configure Gateway API via the {stackstate-product-name} Helm chart

As an alternative to Ingress, the {stackstate-product-name} Helm chart supports the https://gateway-api.sigs.k8s.io/[Kubernetes Gateway API] for exposing {stackstate-product-name} outside the cluster. The chart creates an `HTTPRoute` resource that attaches to an existing `Gateway`.

[NOTE]
====
* A Gateway API-compatible controller (such as https://doc.traefik.io/traefik/routing/providers/kubernetes-gateway/[Traefik], https://gateway.envoyproxy.io/[Envoy Gateway], or https://istio.io/latest/docs/tasks/traffic-management/ingress/gateway-api/[Istio]) must be installed on the cluster.
* A `Gateway` resource must already exist. The chart only creates `HTTPRoute` resources -- it does not create the `Gateway` itself.
* TLS termination is configured on the `Gateway` listener, not on the `HTTPRoute`. Refer to your Gateway controller's documentation for TLS setup.
====

To configure the Gateway API for {stackstate-product-name}, create a file `gateway_values.yaml` with contents like below. Replace `MY_DOMAIN` with your own domain and set the `parentRefs` to match your `Gateway` resource.
...
Generic (Envoy Gateway, Istio, etc.)::
+
--
[,text]
----
...
If `hostnames` is omitted, it will be derived from the `baseUrl` configured during installation. If `path` is omitted, it defaults to the path component of `baseUrl` or `/`.

The `gateway` section also supports optional advanced configuration:

* `annotations` -- annotations for the `HTTPRoute` resource
* `filters` -- https://gateway-api.sigs.k8s.io/reference/spec/#gateway.networking.k8s.io/v1.HTTPRouteFilter[HTTPRoute filters] for request/response modification
* `timeouts` -- request and backend request timeouts
* `backendWeight` -- weight for traffic splitting

Refer to the `gateway` section in the chart's `values.yaml` for full details.

Make sure to update the `baseUrl` in the values file generated during initial installation, it will be used by {stackstate-product-name} to generate convenient installation instructions for the agent.

Include the `gateway_values.yaml` file when you run the `helm upgrade` command to deploy {stackstate-product-name}:

[,text]
----
helm upgrade --install \
  --namespace "suse-observability" \
  --values "gateway_values.yaml" \
  --values $VALUES_DIR/suse-observability-values/templates/baseConfig_values.yaml \
  --values $VALUES_DIR/suse-observability-values/templates/sizing_values.yaml \
  --values $VALUES_DIR/suse-observability-values/templates/affinity_values.yaml \
suse-observability \
...
== Configure Gateway API for Open Telemetry

The OpenTelemetry Collector can also be exposed using the Kubernetes Gateway API. The chart supports both `HTTPRoute` (for OTLP over HTTP, port 4318) and `GRPCRoute` (for OTLP over gRPC, port 4317) via the `kind` field.

To expose both HTTP and gRPC endpoints, use `additionalGateways` to create multiple routes. Create a file `gateway_otel_values.yaml` with contents like below:
...
Include the `gateway_otel_values.yaml` file when you run the `helm upgrade` command to deploy {stackstate-product-name}:
...
helm upgrade \
  --install \
  --namespace "suse-observability" \
  --values "gateway_otel_values.yaml" \
  --values $VALUES_DIR/suse-observability-values/templates/baseConfig_values.yaml \
  --values $VALUES_DIR/suse-observability-values/templates/sizing_values.yaml \
  --values $VALUES_DIR/suse-observability-values/templates/affinity_values.yaml \
suse-observability \
...
* https://gateway-api.sigs.k8s.io/[Kubernetes Gateway API documentation]
* https://doc.traefik.io/traefik/routing/providers/kubernetes-gateway/[Traefik Kubernetes Gateway API (doc.traefik.io)]
