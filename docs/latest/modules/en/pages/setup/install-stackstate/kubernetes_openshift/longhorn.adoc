...
They require its Elasticsearch StorageClass override and Kafka and ZooKeeper per-component precedence over `global.storageClass`.
On earlier releases, use the global durable class and do not apply the mixed-class HA examples below.
====

The following example creates the durable, HA database, and transient classes.
...
Starting with SUSE Observability {next-release-version}, the workload-observer Pod includes a `replication-checker` container by default.
Use it through `kubectl` without installing `sts-backup` locally:

[,bash]
----
kubectl -n observability get pods -l app.kubernetes.io/component=workload-observer
kubectl -n observability exec <workload-observer-pod> -c replication-checker -- \
  sts-backup replication check --namespace observability
----

Replace `<workload-observer-pod>` with the Pod name from the first command.
The chart grants the Pod's service account the required permissions to list Pods and StatefulSets and execute database queries.
Your Kubernetes identity needs access to find and exec into the workload-observer Pod.
Both containers share the service account; its `pods/exec` permission permits arbitrary commands in namespace Pods.
Set `stackstate.components.workloadObserver.replicationChecker.enabled: false` to remove the sidecar and its additional permissions.

Alternatively, install `sts-backup` *v0.10.0 or later*; see xref:/setup/data-management/backup_restore/backup_restore.adoc#_download_the_backup_cli[Download the backup CLI].
...
|===
|Status |Meaning
|`healthy` |The applicable replication checks passed.
|`degraded` |Replication or workload availability has not recovered. Inspect the reported component before continuing maintenance.
|`unknown` |The checker could not verify the state, for example because of permissions, a timeout, or an unsupported configuration. Resolve the reported cause and rerun.
|`not_applicable` |The deployed configuration does not require that replication check, for example a single-replica database or HBase mono. Only Kubernetes availability is checked; this does not establish data redundancy.
|===

=== Wait for recovery during maintenance

Run the wait command before the first node disruption and after each node returns or replacement capacity is ready to schedule workloads.
For an in-place restart, uncordon the returned node first.
Keep the configured database replica counts unchanged and allow only one maintenance workflow to operate on the installation at a time.

The following shell step waits for recovery and stops the workflow on failure:

[,bash]
----
if ! sts-backup replication check \
...
To run this step in the sidecar, replace `sts-backup replication check` with `kubectl -n observability exec <workload-observer-pod> -c replication-checker -- sts-backup replication check`.
The Pod must be available; if it is rescheduling or an exec session is interrupted, wait for it to return and rerun the check before continuing maintenance.
...
Kafka SASL/TLS and Elasticsearch HTTPS customizations are unsupported by this version of the checker.

[IMPORTANT]
====
A successful report describes observed replication; it does not reserve a maintenance window or certify that a particular node can be removed.
Continue checking Longhorn volume health and placement, spare capacity, application health, and backups separately.
The checker does not verify HBase region recovery, durability of the latest active WAL writes, VictoriaMetrics redundancy, or quorum survival after removing the next node.
====
...
. Uncordon the node so workloads can be scheduled, then run <<_wait_for_recovery_during_maintenance,the replication wait command>> and verify Longhorn has restored the expected replica count before proceeding to the next node.
...
. Ensure replacement capacity can schedule workloads, run <<_wait_for_recovery_during_maintenance,the replication wait command>>, and verify Longhorn has restored the expected replica count before removing another node.
...
Run <<_wait_for_recovery_during_maintenance,the replication wait command>> before proceeding to the next node, and confirm Longhorn has restored the expected replica count and the remaining maintenance checks pass.
