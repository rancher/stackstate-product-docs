...
Use `sts-backup` *v0.10.0 or later*; see xref:/setup/data-management/backup_restore/backup_restore.adoc#_download_the_backup_cli[Download the backup CLI].
...
It uses the current kubeconfig context; add `--kubeconfig <path>` to select another configuration.
The Kubernetes identity needs permission to list Pods and StatefulSets and create `pods/exec` requests in the installation namespace.
...
To display the current replication status:
...
if ! sts-backup replication check \
  ...
  --wait --stable-for 30s --interval 10s --timeout 15m \
...
Choose `--timeout` for your recovery window; for large HDFS installations, `--hdfs-audit-timeout 5m` can allow more time for the audit within that overall deadline.
...
Use `sts-backup replication check --help` for connection options when database authentication or TLS is customized.
