...
That release includes the Elasticsearch StorageClass override and Kafka and ZooKeeper per-component precedence over `global.storageClass`.
...
Starting with SUSE Observability {next-release-version}, HA sizing profiles enable a separate `replication-checker` Deployment by default.
...
See the https://github.com/StackVista/stackstate-backup-cli/blob/v0.10.0/docs/replication.md[replication checker documentation] for the full requirements and limitations.
...
Record the original value in your maintenance log and keep the same shell open; capture it only once per maintenance window.
...
If you changed shells, first set `LONGHORN_DRAIN_POLICY` to the original value recorded in your maintenance log; reading the current setting would capture the temporary policy instead.
If the original value is unavailable, confirm the intended policy with your cluster administrator before restoring it:

[,bash]
----
: "${LONGHORN_DRAIN_POLICY:?Set LONGHORN_DRAIN_POLICY to the original value recorded before maintenance.}"
