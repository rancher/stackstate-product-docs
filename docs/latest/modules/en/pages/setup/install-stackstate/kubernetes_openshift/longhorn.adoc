...
Before SUSE Observability {next-release-version}, the `150-ha`, `250-ha`, and `500-ha` sizing profiles configured `__transaction_state` with replication factor one.
This was the only topic with that replication gap; application topics and `__consumer_offsets` already had HA replication.
SUSE Observability {next-release-version} brings `__transaction_state` to replication factor two in those HA profiles and migrates the existing topic during upgrades.
