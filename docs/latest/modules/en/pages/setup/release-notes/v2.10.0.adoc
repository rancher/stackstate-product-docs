...
| *SUSE Observability Agent* | `1.3.18`
|===
...
** The `templates/` directory is now reorganized into per-component subfolders.
** PVC, RBAC, and s3proxy resources now utilize the `common` library chart.
** Non-configurable values have been replaced with named templates and fail-fast validations to prevent invalid overrides. 
** Subchart defaults (for Zookeeper, Kafka, Elasticsearch, ClickHouse, HBase, Victoria Metrics, Anomaly Detection, and OpenTelemetry Collector) have been moved out of the parent `values.yaml` and directly into their respective subcharts. 
** ClickHouse backup resources (CronJob, Service, ConfigMaps) have moved into the `clickhouse` subchart. 
** The `*.enabled` toggles for ClickHouse, HBase, and OpenTelemetry have been removed; these subcharts now install unconditionally.
** *Action Required:* Before upgrading, you must review the xref:setup/release-notes/v2.10.0-helm.adoc[Helm chart migration guide for v2.10.0] and update your `values.yaml` accordingly.
...
** *Action Required:* A full backup *before* upgrading is mandatory. Rolling back to `v1.109.0` is impossible without restoring from a pre-upgrade backup.
...
* *HA Profile Scaling Updates:* ** *500-ha Profile:* Updated the memory settings for the HBase region server and Elasticsearch.
...
=== Backup & Restore
* *StackPack Backups:* Backups now correctly include all active StackPacks, including manually uploaded ones. Previously, only the configuration was backed up, forcing users to re-upload the StackPacks themselves after a restore.
