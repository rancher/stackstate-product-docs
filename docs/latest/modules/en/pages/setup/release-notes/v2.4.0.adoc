...
WARNING: As a result of the recent Java version upgrade, the StringTemplate interface is no longer supported for our use case. Please update all instances of StringTemplate to StsStringTemplate. This change is a necessary step toward removing a temporary workaround in an upcoming release.
