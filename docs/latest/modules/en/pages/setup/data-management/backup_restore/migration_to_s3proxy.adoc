...
[NOTE]
====
The `YOUR_ACCESS_KEY` and `YOUR_SECRET_KEY` values for `s3proxy.credentials` are the internal cluster credentials used by backup jobs to authenticate with S3Proxy. These are not your Azure credentials - set them to any values you choose:
...
As part of this upgrade, a new small PVC (2Gi by default) is created to store settings backups locally. This is separate from the existing settings backup PVC used by previous versions.
