...
[WARNING]
====
*This page describes the legacy backup approach using scripts. For SUSE Observability vX.Y.Z or newer, use the new xref:/setup/data-management/backup_restore/backup_enable.adoc[backup CLI] instead.*
...
The SUSE Observability has a built-in backup and restore mechanism that can be configured to store backups to the local clusters, to AWS S3 or to Azure Blob Storage.
