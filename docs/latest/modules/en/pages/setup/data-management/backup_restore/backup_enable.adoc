...
[NOTE]
====
This page applies to SUSE Observability vX.Y.Z or newer.
====
...
Most backups are enabled with a single Helm value `global.backup.enabled`. Settings backups are always enabled but behave differently based on this value (see below).

=== Backup scope

The following data can be automatically backed up:

* *Configuration and topology data* stored in StackGraph - enabled when `global.backup.enabled` is `true`
...
* <<_kubernetes_persistent_volume,Kubernetes Persistent Volume>>
...
Replace `BUCKET` with your S3 bucket name. The backups for different datastores will be organized using the configured `s3Prefix` values. The same `YOUR_ACCESS_KEY`, `YOUR_SECRET_KEY`, `AWS_ACCESS_KEY`, and `AWS_SECRET_KEY` values from the previous section apply here.
