...
[NOTE]
====
This page applies to {stackstate-product-name} {release-version} or newer. If you are upgrading from a version prior to {release-version}, see xref:/setup/data-management/backup_restore/migration_to_s3proxy.adoc[Migrating to S3Proxy] for important information about changes in configuration of the backup and restore solution.
====
...
** When `global.backup.enabled` is `false`: backups are stored via the S3 Proxy to a small local Persistent Volume
...
Backups use an [S3 Proxy](https://github.com/gaul/s3proxy) as as an S3-compatible gateway to your storage backend. S3 Proxy is automatically deployed by the Helm chart.
...
* `YOUR_ACCESS_KEY` and `YOUR_SECRET_KEY` are the credentials that will be used to secure the S3 Proxy system. The automatic backup jobs and the restore jobs use these to authenticate. They're also required if you want to manually access the S3 Proxy system.
...
To enable backups to cluster-local storage, enable global backups with PVC storage by adding the following YAML fragment to the Helm `values.yaml` file used to install {stackstate-product-name}:
...
**When `global.backup.enabled` is `false`** (backups stored to a local PV):
