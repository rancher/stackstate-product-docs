...
Check the rancher compatibility matrix here xref:setup/install-stackstate/Compatibility Self Hosted.adoc[Rancher compatibility]
...
The `baseUrl` must be the URL via which SUSE Observability will be accessible to Rancher, users, and the SUSE Observability agent. This must match the URL a user will type into the browser to open the UI, so it must include the scheme and any custom ports, for example `+https://observability.internal.mycompany.com+` (this uses the default port). See also <<_accessing_suse_observability,accessing SUSE Observability>>.
...
For a more detailed explanation on how to configure SUSE Observability to use Rancher as an OIDC provider, see xref:/setup/security/authentication/oidc.adoc#_rancher[Configure SUSE Observability for using Rancher as an OIDC provider].
====
