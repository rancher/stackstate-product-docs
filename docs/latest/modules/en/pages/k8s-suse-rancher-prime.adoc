= {ss-rancher-product-short-name}
:revdate: 2025-07-25
:page-revdate: {revdate}
:description: {ss-rancher-product-short-name}

== Introduction

{ss-rancher-product-short-name}, formerly known as StackState can be used for Observability of your Kubernetes clusters and their workloads.

The installation of {ss-rancher-product-short-name}, the {ss-rancher-product-short-name} UI extension and the {ss-rancher-product-short-name} Agents takes about 30 minutes in total.
...
A license key for {ss-rancher-product-short-name} server can be obtained via the SUSE Customer Center in the Subscription tab and will be shown as "{ss-rancher-product-short-name}" Registration Code. This license is valid until the end of your Rancher Prime subscription.

=== Requirements

To install {ss-rancher-product-short-name}, ensure that the cluster has enough CPU and memory capacity. Below are the specific requirements.

There are different installation options available for {ss-rancher-product-short-name}. It is possible to install {ss-rancher-product-short-name} either in a High-Availability (HA) or single instance (non-HA) setup. The non-HA setup is recommended for testing purposes or small environments. For production environments, it is recommended to install {ss-rancher-product-short-name} in a HA setup.
...
These are just the upper and lower bounds of the resources that can be consumed by {ss-rancher-product-short-name} in the different installation options. The actual resource usage will depend on the features used, configured resource limits and dynamic usage patterns, such as Deployment or DaemonSet scaling. For our Rancher Prime customers, we recommend to start with the default requirements and monitor the resource usage of the {ss-rancher-product-short-name} components.
...
{ss-rancher-product-short-name} uses persistent volume claims for the services that need to store data. The default storage class for the cluster will be used for all services unless this is overridden by values specified on the command line or in a `values.yaml` file. All services come with a pre-configured volume size that should be good to get you started, but can be customized later using variables as required.

[NOTE]
====
{ss-rancher-product-short-name} requires the underlying storage to be based on flash memory (SSD) or similar in performance.
====

[NOTE]
====
For production environments, NFS is not recommended and supported for storage provisioning in {ss-rancher-product-short-name} due to the potential risk of data corruption.
====
...
{ss-rancher-product-short-name} is installed through Helm, which needs to be installed with a minimum version of 3.13.1.

=== The different components

==== {ss-rancher-product-short-name} Server
...
==== {ss-rancher-product-short-name} Agent

The lightweight {ss-rancher-product-short-name} agent is installed on your downstream worker nodes. It collects and reports metrics, events, traces and logs, and it provides real-time observability and insights, enabling proactive monitoring and troubleshooting of your IT environment.

The {ss-rancher-product-short-name} version of the Agent also uses eBPF as a lightweight way to monitor all your workloads and their communication. It also decodes the RED (Rate, Errors and Duration) signals for most of the common L7 protocols like TCP, HTTP, TLS, Redis, etc.

==== Rancher Prime - Observability UI extension

This is an UI extension to Rancher Manager that integrates the health signals observed by {ss-rancher-product-short-name}. It gives direct access to the health of any resource and a link to {ss-rancher-product-short-name}'s UI for further investigation.

=== Where to install {ss-rancher-product-short-name} server

{ss-rancher-product-short-name} server should be installed in its own downstream cluster intended for Observability. See the below picture for reference.

For {ss-rancher-product-short-name} to be able to work properly it needs:
...
* the observability cluster to support a way to expose {ss-rancher-product-short-name} on an HTTPS URL to Rancher, {ss-rancher-product-short-name} users and the {ss-rancher-product-short-name} agent. This can be done via an Ingress configuration using an ingress controller, alternatively a (cloud) loadbalancer for the {ss-rancher-product-short-name} services could do this too, for more information see the https://ranchermanager.docs.rancher.com/how-to-guides/new-user-guides/kubernetes-resources-setup/load-balancer-and-ingress-controller[Rancher docs].

image::k8s/prime/SUSEObservabilityDeployment.png[Architecture]

=== Pre-Installation

Before installing the {ss-rancher-product-short-name} server a default storage class must be set up in the cluster where the {ss-rancher-product-short-name} server will be installed:
...
* *For RKE2 Node Drivers*: No storage class is created by default. You will need to create one before installing {ss-rancher-product-short-name}.

== Installing {ss-rancher-product-short-name}
...
After meeting the prerequisites you can proceed with the installation. The installation is NOT YET AVAILABLE from the app store. Instead, you can install {ss-rancher-product-short-name} via the kubectl shell of the cluster.
...
global:
  ...
  suseObservability:
    ...
    # Required: Base URL for {ss-rancher-product-short-name}
...
The `baseUrl` must be the URL via which {ss-rancher-product-short-name} will be accessible to Rancher, users, and the {ss-rancher-product-short-name} agent. The URL must include the scheme, for example `+https://observability.internal.mycompany.com+`. See also <<_accessing_suse_observability,accessing {ss-rancher-product-short-name}>>.
...
[warn]
=====
Using a single default password is great to get started with {ss-rancher-product-short-name}, but for a production setup xref:/setup/security/authentication/authentication_options.adoc[more secure authentication options] are available.
=====
...
This command generates the files `$VALUES_DIR/suse-observability-values/templates/baseConfig_values.yaml`, `$VALUES_DIR/suse-observability-values/templates/sizing_values.yaml`, and `$VALUES_DIR/suse-observability-values/templates/affinity_values.yaml` containing the necessary configuration to install the {ss-rancher-product-short-name} Helm Chart.

[NOTE]
=====
The {ss-rancher-product-short-name} administrator password will be autogenerated by the above command and are output as comments in the generated `basicConfig.yaml` file. For more info, see xref:/setup/security/authentication/single_password.adoc[single password].
...
[NOTE]
=====
Store the generated `basicConfig.yaml`, `sizing_values.yaml` and `affinity_values.yaml` files safely. You can reuse these files for upgrades, which saves time and ensures that {ss-rancher-product-short-name} continues to use the same API key. This is desirable as it means Agents and other data providers for {ss-rancher-product-short-name} won't need to be updated.
...
[NOTE]
=====
The {ss-rancher-product-short-name} Values chart generates affinity configurations you can use with the main {ss-rancher-product-short-name} chart to control pod scheduling behavior. Refer to xref:/setup/install-stackstate/kubernetes_openshift/affinity.adoc[Configure Kubernetes Affinities] for more information.
=====

. Configure {ss-rancher-product-short-name} to use Rancher as an OIDC provider.
...
For a more detailed explanation on how to configure {ss-rancher-product-short-name} to use Rancher as an OIDC provider, see xref:/setup/security/authentication/oidc.adoc#_rancher[Configure {ss-rancher-product-short-name} for using Rancher as an OIDC provider].
====

. Deploy the {ss-rancher-product-short-name} helm chart with the generated values:
...
== Accessing {ss-rancher-product-short-name}

The {ss-rancher-product-short-name} Helm chart has support for creating an Ingress resource to make {ss-rancher-product-short-name} accessible outside of the cluster. Follow xref:/setup/install-stackstate/kubernetes_openshift/ingress.adoc[these instructions] to set that up when you have an ingress controller in the cluster. Make sure that the resulting URL uses TLS with a valid, not self-signed, certificate.
...
. Once installed, on the left panel of the rancher UI, the _{ss-rancher-product-short-name}_ section appears.
. Navigate to the _{ss-rancher-product-short-name}_ section and select "configurations". In this section, you can add the {ss-rancher-product-short-name} server details and connect it.
. Follow the instructions as mentioned in _Obtain a service token_ section below and fill in the details.

=== Obtain a service token:

. Log into the {ss-rancher-product-short-name} instance.
. From the top left corner, select CLI.
. Note the API token and install {ss-rancher-product-short-name} cli on your local machine.
...
=== {ss-rancher-product-short-name} Rancher UI extension compatibility matrix
...
== Installing the {ss-rancher-product-short-name} Agent

. In the {ss-rancher-product-short-name} UI open the main menu and select StackPacks.
...
. After you install the agent, the cluster can be seen within the {ss-rancher-product-short-name} UI as well as the _SUSE Rancher - Observability UI extension_.

== Required privileges

The deployment of the {ss-rancher-product-short-name} agent requires the following system privileges:
...
. Is it mandatory to install a {ss-rancher-product-short-name} agent before proceeding with adding the UI extension?
 ** No this is not mandatory, the UI extension can be installed independent.
. Is it mandatory to install {ss-rancher-product-short-name} Server before we proceed with UI extensions?
 ** Yes this is mandatory since you need to provide a {ss-rancher-product-short-name} endpoint in the configuration
. Can we install {ss-rancher-product-short-name} on a local cluster or on a downstream cluster?
 ** Both options are possible.
. To monitor the downstream clusters, should we install the {ss-rancher-product-short-name} agent from the app store or add a new instance from the {ss-rancher-product-short-name} UI?
...
. After the extensions are installed, the {ss-rancher-product-short-name} UI opens in the same tab as the Rancher UI.
