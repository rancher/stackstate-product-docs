...
To mirror your Prometheus metrics in SUSE Observability, you need to lookup the service token that is used to send in metrics into SUSE Observability. The service token can be found in the description of the installed Kubernetes StackPack in SUSE Observability. The following steps show how to find the service token:

. Open the SUSE Observability UI and navigate to the StackPacks page
+
image::k8s/k8s-stackpacks.png[StackPacks]

. Find the Kubernetes StackPack and click it.
. In the description of the StackPack, you will find the service token that's used to send in metrics into SUSE Observability.
+
image::k8s/k8s-stackpacks-apikey.png[service token]

== Configuring your Prometheus instance

[IMPORTANT]
====
A service token must be used instead of API_KEY as they are synonymous. The environment variable remains referred to API_KEY in below code snippets for backwards compatibility.
====
...
You can authenticate using the service token by adding the `sts-api-key` header to the prometheus remote_write requests. The following example shows how to configure this:
