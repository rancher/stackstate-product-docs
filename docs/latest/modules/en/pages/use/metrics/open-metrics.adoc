...
Retrieved metrics are not be automatically be mapped to topology elements but they can be browsed using the xref:/use/metrics/k8sTs-explore-metrics.adoc[telemetry inspector] and eventually xref:/setup/custom-integrations/metric-bindings/index.adoc[added to components via a metric binding].
