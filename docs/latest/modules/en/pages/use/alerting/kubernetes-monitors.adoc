...
If you have a use case where logical components have no direct monitors then you can use the xref:/setup/custom-integrations/monitors/derived-state-monitors.adoc[Derived State Monitor] function to infer their health based on the technical components they depend on.

== See also

* xref:/use/alerting/k8s-monitors.adoc[Monitors]
* xref:/setup/custom-integrations/monitors/schemas-ref.adoc[Monitor schema]
