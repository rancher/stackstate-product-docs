...
Webhooks are custom HTTP callbacks that you define and run. They can take any action needed whenever a notification is opened or closed. For example, by creating a ticket in a ticketing system that is not supported natively by SUSE Observability. Or by simply writing the notification messages to an S3 bucket for future reference.
...
. Secret token - a secret token that SUSE Observability includes in every request to <<validate-the-requests,validate it>>
. Metadata - add extra key/value pairs that are included in the payload. This can be used when the same endpoint handles multiple SUSE Observability webhooks and needs extra information
...
. Monitor: the monitor that triggered the notification. Next to the monitor name, tags and identifier also a link is included. The link opens the monitor in the SUSE Observability UI.
...
. Metadata: It is possible to specify metadata on a webhook channel. The metadata is one-to-one reproduced here as a set of key/value pairs.
...
* The triggering health state is muted (or unmuted) by a change in silencing rules.
...
The example does not authenticate the request, which can be added by verifying the value of the <<validate-the-requests,token header>>.
