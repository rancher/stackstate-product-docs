...
. Fill in the parameters for each monitor type, such as metrics, monitoring conditions, component mappings, and the like, as per your monitoring requirements.

. Click 'SAVE CHANGES' to add the monitor to the component.

== Example: Adding a Metric Threshold Monitor

In this example, we add a metric threshold monitor to a component in {stackstate-product-name} to trigger a health state when a given threshold is exceeded for a specified metric query.
...
[%collapsible]
====
For information on writing a remediation guide, see xref:/setup/custom-integrations/monitors/remediation-guide.adoc[Write a remediation guide].
====
+
