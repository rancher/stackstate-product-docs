...
* <<how-to-build-my-annotation,How to build my annotation>>
* <<what-monitors-allow-overriding_arguments,What monitors allow overriding arguments?>>
* <<build-an-override-for-a-custom_monitor,Build an override for a custom monitor>>
...
----
monitor.${owner}.stackstate.io/${monitorShortName}
----

The `owner` property represents who created such a monitor, for the out of the box monitors is `kubernetes-v2`, and the `monitorShortName` property represents the id of the monitor and can be extracted from the `identifier` property of a monitor which can be read from the cli when listing or inspecting monitors
...
In our example the identifier is `urn:stackpack:kubernetes-v2:shared:monitor:kubernetes-v2:service-available-endpoint` and the `monitorShortName` corresponds to the very last segment as in `service-available-endpoint` therefore the annotation key is:
...
[#what-monitors-allow-overriding_arguments]
...
[#build-an-override-for-a-custom_monitor]
== Build an override for a custom monitor

Any custom threshold monitor is suitable to override arguments, as xref:/setup/custom-integrations/monitors/schemas-ref.adoc#schema-for-monitor[the monitor schema shows] an identifier for a custom monitor is structured as ``+urn:stackpack:<stackpack-name>:monitor:<monitor-short-name>+`` and the override annotation key for such an identifier is expected to be

[,bash]
----
monitor.<stackpack-name>.stackstate.io/<monitor-short-name>
----

For a monitor with identifier `urn:stackpack:custom:monitor:deployment-has-replicas` therefore the annotation key would be
