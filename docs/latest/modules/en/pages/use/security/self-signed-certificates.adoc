...
=== Exposing {stackstate-product-name} Server with self-signed certificates

xref:k8s-suse-rancher-prime.adoc#_installation[{stackstate-product-name} Server] components cannot terminate SSL themselves and must use an SSL-protected Ingress Controller or LoadBalancer to expose the service with HTTPS. This section explains how to configure an Ingress Controller with self-signed certificates.

==== Using Ingress Controller with TLS

To expose {stackstate-product-name} Server using https://kubernetes.github.io/ingress-nginx/[Nginx Ingress Controller] with self-signed certificates, you need to:

1. Create Kubernetes TLS secrets from your server certificate and key
2. Configure the Ingress resource with TLS settings
3. Deploy {stackstate-product-name} Server with Ingress enabled
...
[,bash]
----
# Create TLS secret for main {stackstate-product-name} Server
...
Deploy {stackstate-product-name} Server using the Ingress configuration:
...
=== {stackstate-product-name} Server connecting to external systems with self-signed certificates

[NOTE]
====
This section is for configuring {stackstate-product-name} Server to trust external systems that use self-signed certificates. This applies when the server needs to make outgoing connections to external services (like webhooks) that are secured with self-signed certificates.
====

{stackstate-product-name} Server has several points of interaction with external systems. For example, event handlers can call out to webhooks in other systems. With the default configuration, {stackstate-product-name} Server does not communicate with these systems if they are secured with TLS using a self-signed certificate, or a certificate that is not, by default, trusted by the JVM.

To mitigate this, {stackstate-product-name} Server allows configuration of a custom trust store.

=== Create a custom trust store

You need to have the custom TLS certificate available. If you do not have that, you need to xref:/use/security/self-signed-certificates.adoc#_retrieve_certificate_via_the_browser[retrieve it via the browser].

Use the keytool tool and the `cacerts` file included in the JVM (Java Virtual Machine) installation to convert an existing TLS certificate file to the format needed by {stackstate-product-name} Server. You can run this on any machine, regardless of the type of operating system.
...
. The `custom_cacerts` store file now includes the `site.cert` certificate. You can verify that by searching for the alias in the output:
+
[,bash]
----
 keytool -list -keystore custom_cacerts
...
* The first run of the helm upgrade command restarts the pods which may cause a short interruption of availability.
...
xref:k8s-suse-rancher-prime.adoc#_installing_the_suse_observability_agent[The {stackstate-product-name} Agent] connects to the {stackstate-product-name} Server via HTTPS. If your server uses a self-signed certificate, you must configure the Agent to trust this certificate to establish secure connections.
...
== {stackstate-product-name} CLI

xref:setup/cli/cli-sts.adoc[The {stackstate-product-name} CLI] connects to the {stackstate-product-name} Server via HTTPS. When your server uses self-signed certificates or certificates from a private Certificate Authority (CA), configure the CLI to trust these certificates.
...
== {stackstate-product-name} Open Telemetry Collector Exporter
...
. Update the `otel-collector.yaml` with the below configuration and mount the configmap at the top-level:
...
== Rancher UI extension for {stackstate-product-name}

When installing the Rancher UI extension for {stackstate-product-name} (see xref:/k8s-suse-rancher-prime.adoc#_installing_ui_extensions[Installing UI extensions]), the extension must communicate with your {stackstate-product-name} Server. If your server uses self-signed certificates, the extension installation will fail.
...
After configuring the certificate in Rancher, the extension connect to your {stackstate-product-name} Server.
