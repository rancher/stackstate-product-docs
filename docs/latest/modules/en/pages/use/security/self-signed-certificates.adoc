...
[,bash]
----
for release in suse-observability-agent suse-observability; do
  if helm get manifest "$release" --namespace suse-observability 2>/dev/null | grep -q 'helm.sh/chart: suse-observability-agent-'; then
    helm uninstall "$release" --namespace suse-observability
  fi
done;
helm upgrade --install \
