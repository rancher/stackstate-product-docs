...
[NOTE]
====
Note that logs stored on pods are regularly removed. For long term access to logs, it is advised that you set up <<log-aggregation,Log Aggregation>> for your Kubernetes cluster.
====
...
The most recent logs can be retrieved from Kubernetes using the `kubectl logs` command. Check the <<kubernetes-pods-for-logging,Kubernetes pods for logging>> to retrieve a specific log.
