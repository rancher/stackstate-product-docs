...
[IMPORTANT]
====
A service token must be used instead of api key as they are synonymous. Any environment variable that continues to refer to `STACKSTATE_RECEIVER_API_KEY` is only for backwards compatibility.
====
