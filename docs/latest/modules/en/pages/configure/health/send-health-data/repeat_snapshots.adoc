...
ifdef::ss-ff-silencing_enabled[]
 ** *silenced* - Optional object with a required `note` string. Marks this result as muted. See xref:/configure/health/send-health-data/send-health-data.adoc#_silence_an_external_health_result[Silence an external health result].
endif::ss-ff-silencing_enabled[]
