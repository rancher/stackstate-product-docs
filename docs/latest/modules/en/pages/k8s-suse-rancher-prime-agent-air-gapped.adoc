...
The default namespace for the SUSE Observability Agent is `suse-observability-agent`. The command below removes a previous agent release from `suse-observability` before installing into the new namespace to avoid running duplicate agents.
...
[,bash]
----
kubectl get pod -n suse-observability-agent
----
