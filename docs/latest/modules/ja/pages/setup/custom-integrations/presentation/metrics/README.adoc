...
. <<reate-an-outline-of-the-metric-binding,Create an outline of the metric binding>>
...
. <<attach-the-metric-to-a-ui-location,Attach the metric to a UI location>>
...
Open the presentation YAML file in your favorite code editor to change it throughout this guide. You can use `sts stackpack test-deploy` to test the StackPack, as described in xref:/setup/custom-integrations/create.adoc#_test_deploy_the_stackpack[Test deploy the StackPack].
...
In the above setting, the metric is shown in the metrics perspective; alternative locations are <<attach-the-metric-to-a-ui-location,shown below>>.
The metricQueries section will be filled in the next steps. 
