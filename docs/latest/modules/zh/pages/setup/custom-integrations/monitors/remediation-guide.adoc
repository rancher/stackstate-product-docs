...
{stackstate-product-name} 提供 xref:/use/alerting/k8s-monitors.adoc[开箱即用的监控]，可对 Kubernetes 集群中可能发生的常见问题进行监控。这些监控还包含开箱即用的补救指南，旨在指导用户准确排除问题。它们是基于最佳实践和社区知识创建的。按照本页面上的指示，了解如何自己编写有效的补救指南。
